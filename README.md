...
•	Out-of-Band Setup (IDS): The IDS sensor is wired to a SPAN or mirror port on the core switch. It only receives duplicate frames. If the IDS device fails, crashes, or gets overloaded, live user traffic continues through the switch without interruption. 
